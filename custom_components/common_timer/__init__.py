...
from homeassistant.components.template.sensor import (
    SensorTemplate,
    PLATFORM_SCHEMA as SENSOR_TEMPLATE_PLATFORM_SCHEMA,
)
        ...
        DOMAIN: vol.Schema(
            {
                ...
                vol.Optional(CONF_EXCLUDE, default=[]): vol.All(
                    cv.ensure_list, vol.Length(min=0), [cv.string]
                ),
                ...
                vol.Optional(
                    CONF_LINKED_USER, default="common_timer_linked_user"
                ): cv.string,
...
def async_setup(hass, config):
    ...
    for setup_domain in ["input_select", "input_text", "input_boolean", "sensor"]:
        ...
        if setup_domain in components or setup_domain in hass.data:
            _LOGGER.debug(
                "initialize component[%s]: config has this component", setup_domain
            )
            ...
            if setup_domain in [
                ...
                "input_text",
                "input_boolean",
            ]:  # entity belongs to component
                _LOGGER.debug(
                    "initialize component[%s]: component is ready, use EntityComponent's method to initialize entity.",
                    setup_domain,
                )
                ...
                for object_id, conf in VALIDATED_CONF.get(setup_domain, {}).items():
                    ...
                    if setup_domain == "input_select":
                        ...
                        _LOGGER.debug(
                            "input_boolean.timer_button:%s,%s,%s,%s",
                            object_id,
                            conf.get(CONF_NAME),
            ...
            # sensor should set a unique namespace to ensure it's a new platform and don't affect other entities using template platform which have been initialized.
                ...
                # SCHEMA
                p_validated = SENSOR_TEMPLATE_PLATFORM_SCHEMA(
                    VALIDATED_CONF.get(setup_domain, {})[0]
                )
                ...
                yield from hass.data[setup_domain].async_setup_platform(
                    "template", p_validated
                )
            else:
                _LOGGER.debug(
        ...
        # add config for HA to initailize
            ...
            _LOGGER.debug(
                "initialize component[%s]: config hasn't %s component , use HA's setup method to initialize entity.",
                setup_domain,
                setup_domain,
            )
    ...
    if info_config:
        ...
        yield from hass.data["sensor"]._platforms[PLATFORM_KEY].async_add_entities(
            entities
        )
        data = {
    ...
    def start_common_timer(event):
        ...
        for user in users:
            if user.name == username:
                ...
                break
        if CONTEXT is None:
            user = yield from hass.auth.async_create_system_user(
                username, [GROUP_ID_ADMIN]
            )
            CONTEXT = Context(user.id)
        ...
        common_timer = CommonTimer(
            domains, exclude, pattern, ratio, ui, hass, info_config
        )
        ...
        def common_timer_handle(event):
            ...
            if event.data[ATTR_ENTITY_ID] == ui[UI_INPUT_DOMAIN]:
                ...
                common_timer.choose_operation(
                    event.data["new_state"].as_dict()["state"], context=event.context
                )
                ...
                if interrupt_loop and common_timer.stop_loop_task(
                    event.data[ATTR_ENTITY_ID], context=event.context
                ):
...
class CommonTimer:
    ...
    def __init__(
        self, domains, exclude, pattern, ratio, ui, hass=None, info_config=None
    ):
        ...
        self._dic_operation_cn_to_en = {
            v: k for k, v in self._dic_operation_en_to_cn.items()
        }
        # 改动 增加类别
    ...
    def refresh_ui(self):
        ...
        self.set_state(
            self._ui[UI_INPUT_OPERATION],
            state={"option": self.get_operation(task=task)},
            service="select_option",
            context=CONTEXT_IGNORE,
        )
    ...
    def start(self):
        ...
        tasks = (
            {
                user_dict["entity_id"]: {
                    ...
                    "duration": user_dict.get("duration", "0:00:00"),
                    "operation": user_dict.get("operation", "off"),
                    "count": user_dict.get("count", 0),
                    "ratio": user_dict.get("ratio", self._ratio),
                }
                for user_dict in data["tasks"]
                if "entity_id" in user_dict
            }
            if data
            else {}
        )
        ...
        for state in states:
            ...
            if domain not in self._domains or entity_id in self._exclude:
                ...
                if not self._pattern or pattern.search(friendly_name):
                    ...
                    self._tasks[domain][entity_id]["icon"] = self.get_attributes(
                        entity_id
                    ).get("icon", self._dic_icon[domain])
                    ...
                    if tasks.get(entity_id) is not None:
                        self._tasks[domain][entity_id]["duration"] = tasks.get(
                            entity_id
                        ).get("duration")
                        self._tasks[domain][entity_id]["operation"] = tasks.get(
                            entity_id
                        ).get("operation")
                        self._tasks[domain][entity_id]["count"] = tasks.get(
                            entity_id
                        ).get("count")
                        self._tasks[domain][entity_id]["ratio"] = tasks.get(
                            entity_id
                        ).get("ratio")
                    else:
                        self._tasks[domain][entity_id]["duration"] = "0:00:00"
                        self._tasks[domain][entity_id]["operation"] = (
                            "on"
                            if domain == "autonmation" or domain == "script"
                            else "off"
                        )
        ...
        self._hass.async_add_job(
            self._hass.services.async_call("input_select", SERVICE_SET_OPTIONS, data)
        )
    ...
    def save_tasks(self):
        ...
        tasks = [
            ...
            for entities in self._tasks.values()
            for entity_id, attrs in entities.items()
            if attrs["duration"] != "0:00:00" or attrs["count"] != 0
        ]
    ...
    def choose_domain(self, domain):
        ...
        if domain == "---请选择设备类型---":
            ...
            options = [
                self._get_task(entity["entity_id"])["friendly_name"]
                for entity in sorted(
                    entities, key=operator.itemgetter("count"), reverse=True
                )
            ]  # show friendly_name
    ...
    def choose_entity(self, friendly_name):
        ...
        if friendly_name == "---请选择设备---":
            ...
            entity_id = self._entity_id = self._dic_friendly_name.get(
                friendly_name, None
            )
            ...
            options = [
                "开",
                "关",
                "开⇌关 [1:{}]".format(task["ratio"]),
                "关⇌开 [1:{}]".format(task["ratio"]),
                "调服务",
            ]
    ...
    def switch(self, state, context=None):
        ...
        if self._domain != "---请选择设备类型---":
            ...
            if task is None:
                ...
                operation = self.get_operation(
                    ui_operation=self.get_state(self._ui[UI_INPUT_OPERATION])
                )
                ...
                if state == "on":
                    ...
                    if task["handle"] is None:
                        ...
                        task["handle"] = self._queue.insert(
                            entity_id, duration, self.handle_task, operation=operation
                        )  # initialize queue task
                        task["operation"] = operation  # set operation attr

                        # sync state for loop task
                        if "temporary" in operation:
                            task["next_operation"] = operation.split("_")[
                                1
                            ]  # set next_operation attr, used in info panenl to show state
                            state = "off" if task["next_operation"] == "on" else "on"
                            self.set_state(
                                entity_id, service="turn_" + state, force_update=True
                            )
                            # service.call()
                        else:
                        ...
                        task[
                            "exec_time"
                        ] = datetime.now() + self._queue.get_remaining_time(
                            task["handle"]
                        )
                # stop timer
                else:
                    ...
                    if "temporary" in task["operation"]:
                        ...
                        self.set_state(
                            self._ui[UI_INPUT_DURATION], state=task["duration"]
                        )  # reset control panel ui
                    _LOGGER.debug("---switch---")
                    self.set_state(
                        self._ui[UI_INPUT_OPERATION],
                        state=self.get_operation(task=task),
                    )  # reset control panel ui
        ...
        if (
            context != CONTEXT_IGNORE
        ):  # UI_SWITCH will be reset after a task finish, this shouldn't trigger a update twice
    ...
    def get_operation(self, ui_operation=None, task=None):
        ...
        if task is not None:
            ...
            if task["operation"] in ["on", "off"]:
                ...
                return "{} [1:{}]".format(
                    self._dic_operation_en_to_cn.get(task["operation"]), task["ratio"]
                )
        else:
            return "关"
    ...
    def set_state(
        ...
        if service is None:
            _LOGGER.debug(
                "[set_state] state machine: entity_id= {}, from {} to {}, context = {}".format(
                    entity_id, self.get_state(entity_id), state, context
                )
            )
            ...
            self._hass.states.async_set(
                entity_id, state, attr, force_update=force_update, context=context
            )
        else:
            domain = entity_id.split(".")[0]
            _LOGGER.debug(
                "[set_state] call service: entity_id =%s, context = %s",
                entity_id,
                context,
            )
            ...
            self._hass.async_add_job(
                self._hass.services.async_call(domain, service, data, context=context)
            )

    def set_options(
        self, entity_id, options, current_option=None, context=CONTEXT_IGNORE
    ):
        ...
        self._hass.async_add_job(
            self._hass.services.async_call(
                domain, SERVICE_SET_OPTIONS, data, blocking=True, context=context
            )
        )  # set blocking to wait till service is executed
    ...
    def update(self, time):
        ...
        if self.get_state(self._ui[UI_SWITCH]) == "on":
            ...
            if entity_id is None:
                _LOGGER.info(
                    "Function task: friendly_name(%s) not found in dic !", entity_id
                )
                return
            task = self._get_task(entity_id)
            remaining_time = self._queue.get_remaining_time(task["handle"])
            ...
            if remaining_time is None:
                ...
                if remaining_time == "0:00:00":
                    ...
                    self.set_state(
                        self._ui[UI_INPUT_DURATION], state=str(remaining_time)
                    )
    ...
    def handle_task(self, entity_id, command, **kwargs):
        ...
        """
        _LOGGER.debug(
            "[handle_task] start: entity_id = %s, command = %s.", entity_id, command
        )
        task = self._get_task(entity_id)
        task["handle"] = None
        task["remaining"] = "0:00:00"
        ...
        if mode == "custom":
            try:
                if operation == "*":
                    _LOGGER.debug(
                        "[handle_task] custom mode: entity has no custom command, skip"
                    )
                    return
                try:
                    ...
                    for attr in operation.split("/"):
                        cmnds = cmnds[attr]
                    _LOGGER.debug(
                        "[handle_task] custom mode: get custom command %s", cmnds
                    )
                except:
                    _LOGGER.error(
                        "[handle_task] custom mode: entity has no attr %s", operation
                    )
                    return
                if not isinstance(cmnds, list) or len(cmnds[0]) != 3:
                    _LOGGER.error(
                        "[handle_task] custom mode: service command format error %s",
                        cmnds,
                    )
                    return
                translation = lambda cmnds: (
                    [cmnd[0] for cmnd in cmnds],
                    [cmnd[1] for cmnd in cmnds],
                    [json.loads(cmnd[2]) for cmnd in cmnds],
                )
                ...
                for i, d in enumerate(data_list):
                    if (
                        "entity_id" not in d
                        and domain_list[i] not in DOMAIN_SERVICE_WITHOUT_ENTITY_ID
                    ):
                        d.update({"entity_id": entity_id})
                for i in range(len(domain_list)):
                    _LOGGER.debug(
                        "domain: %s, servcie: %s, data: %s",
                        domain_list[i],
                        service_list[i],
                        data_list[i],
                    )
            ...
            if operation == "temporary_on":
                ...
                task["exec_time"] = datetime.now() + self._queue.get_remaining_time(
                    task["handle"]
                )
                operation = "on"
            ...
            self._hass.async_add_job(
                self.call_service_and_update_info, entity_id, service
            )
    ...
    def stop_loop_task(self, entity_id, context):
        ...
        if self._get_index_of_running_tasks(entity_id) is not None:
            ...
            if "temporary" in task["operation"] and context.user_id != CONTEXT.user_id:
                _LOGGER.debug(
                    "operated by other method. <entity_id = %s, context = %s, common_timer context = %s>",
                    entity_id,
                    context,
                    CONTEXT,
                )
                ...
                _LOGGER.debug(
                    "operated by common_timer.py. <entity_id = %s, context = %s>",
                    entity_id,
                    context,
                )
            return True
    ...
    def _get_running_tasks(self):
        ...
        tasks = [
            attrs
            for domain_entities in self._tasks.values()
            for entity_id, attrs in domain_entities.items()
            if attrs["handle"] is not None
        ]
    ...
    def update_info(self, entity_id=None):
        ...
        info_row_num = (
            len(running_tasks)
            if len(running_tasks) < info_config[CONF_INFO_NUM_MAX]
            else info_config[CONF_INFO_NUM_MAX]
        )
        new_rows = []
        info_ui = []
        ...
        for row in range(0, info_config[CONF_INFO_NUM_MAX]):
            ...
            if row < info_row_num:
                ...
                if "custom:" in running_tasks[row]["operation"]:
                    ...
                    loop_flag = (
                        CONF_LOOP_FLAG
                        if "temporary" in running_tasks[row]["operation"]
                        else ""
                    )
                    info2 = Template(
                        "{} {} → {}".format(
                            loop_flag,
                            self.get_state(running_tasks[row]["entity_id"]),
                            running_tasks[row]["next_operation"],
                        )
                    )  # operation info template
                ...
                # row has record, add
                    ...
                    _LOGGER.debug(
                        ...
                        row,
                        info_entity_id,
                        self.get_operation(running_tasks[row]["operation"]),
                    )
            ...
            # rows show blank or should be remove
                ...
                if (
                    not any([info_row_num, row])
                    or row < info_config[CONF_INFO_NUM_MIN]
        ...
        if new_rows:
            yield from self._hass.data["sensor"]._platforms[
                PLATFORM_KEY
            ].async_add_entities(new_rows, update_before_add=True)
    ...
    def set_task(self, entity_id, operation, duration, is_loop):
        ...
        if task is not None:
            ...
            if self._entity_id == entity_id:
                _LOGGER.debug("---set_task---")
                self.set_state(
                    self._ui[UI_INPUT_OPERATION], state=self.get_operation(task=task)
                )  # reset control panel ui
                self.set_state(self._ui[UI_INPUT_DURATION], state=task["duration"])
                self.set_state(self._ui[UI_SWITCH], state="on")
        else:
...
class DelayQueue(object):
    ...
    def insert(self, task_id, duration, callback, operation="turn_off", **kwargs):
        ...
        delayQueueTask = DelayQueueTask(
            task_id, operation, int(slot), loop, callback, kwargs=kwargs
        )
        self.__queue[delayQueueTask.slot].append(delayQueueTask)
        _LOGGER.debug(
            "create task:{}/{}".format(delayQueueTask.slot, delayQueueTask.loop)
        )
    ...
    def get_remaining_time(self, delayQueueTask):
        ...
        if delayQueueTask:
            if (
                self.__current_slot - 1 > delayQueueTask.slot
                and self.__current_slot - 1 < 60
            ):
                second = (
                    self.__slots_per_loop * (delayQueueTask.loop + 1)
                    + delayQueueTask.slot
                    - (self.__current_slot - 1)
                )
            else:
                second = (
                    self.__slots_per_loop * delayQueueTask.loop
                    + delayQueueTask.slot
                    - (self.__current_slot - 1)
                )
